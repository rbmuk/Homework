...
\begin{document}
    ...
    Now suppose that both conditions are false. Then we cannot possibly have a PSD matrix in $\widehat{\mathcal A^*}$, because that would satisfy condition (i), but condition 1 is false. Thus we have a PSD matrix in both $\widehat A$ and $\widehat A^*$. Call the PSD matrix in the first set $X$ and the one in the second set $M = \sum x_iA_i$. Now we have that $M \cdot X = 0$ obviously. Diagonlize $X = \sum \lambda_i v_iv_i^T$. Then we see that $M \cdot X = \sum \lambda_i M \cdot v_iv_i^T = 0$. Again $Mv_i = 0$ for each $i$. This tells us that,
    \begin{align*}
