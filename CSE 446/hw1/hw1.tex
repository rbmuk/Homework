...
\begin{document}
    ...
    \begin{enumerate}[leftmargin=\labelsep]
        ...
        \item \begin{enumerate}
            ...
            \begin{align*}
                \grad_{w_j} \mg{Xw_j-Ye_j}_2^2 &= \grad_{w_j} (Xw_j-Ye_j)^T(Xw_j-Ye_j) \\
                &= \grad_{w_j} w_j^TX^TXw_j - 2e_j^TY^TXw_j \\
                &= 2X^TXw_j - 2X^TYe_j
            \end{align*}
            That is, the $j$th column of $\grad_W f(W)$ is $2X^TXw_j - 2X^TYe_j$, so $\grad_W f(W) = 2X^TXW - 2X^TY$. Thus the gradient of $g(W) = F(W) + \lambda \mg{W}_F^2$ is just $2X^TXW - 2X^TY + 2\lambda W$. Setting this to zero shows that $(X^TX+\lambda I)W = X^TY$, which shows that $\hat{W} = (X^TX+\lambda I)^{-1}X^TY$.
            ...
            \end{figure}
            The big pattern I notice is that most of the numbers that it wrongly predicted seemed to be tilted left or right or disconnected. I would guess that most of the data in MNIST does not have these features, so it is definitely expected that it would get these wrong.
        \end{enumerate}

        \item This assignment took me around 7 hours.
    \end{enumerate}
\end{document}
