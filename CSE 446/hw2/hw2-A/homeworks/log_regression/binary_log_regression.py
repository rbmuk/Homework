...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        return 1 / (1+np.exp(-y*(X @ self.weight + self.bias)))
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        return np.mean(np.log(1+np.exp(-y*(X @ self.weight + self.bias)))) + self._lambda * np.sum(self.weight ** 2)
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        return np.mean(((self.mu(X, y) - 1) *  y).reshape(-1, 1) * X, axis=0) + 2 * self._lambda * self.weight
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        prediction = np.sign(X @ self.weight + self.bias)
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, batch_size=1, epochs=30, learning_rate=1e-4)
