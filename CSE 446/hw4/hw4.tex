...
\begin{document}
    ...
    \begin{enumerate}[label=\alph*.]
        ...
        \item This is not true. We have in general that every row of $V$ is an eigenvalue of $X^\top X$, however the reverse inclusion is in general false. Since $V$ has size $r \times n$ for $r = \rank X^\top X$, the rows of $V$ are only the first $r$ eigenvalues of $X^\top X$, not all of them.
        ...
        \item The SVD of a matrix is in general not unique. We could permute the columns of $U$, rows of $V$, and entries of $S$ (by the same permutation) to get another singular value decomposition. 
        \item This is false. Consider the matrix $aI \in \R^{n \times n}$ for some nonzero constant $a \in \R$. This matrix obviously has rank $n$, being invertible (with inverse $a^{-1} I$), but this matrix has only one eigenvalue $a$.
    \end{enumerate}
    ...
    \subsection*{A3.}
    \begin{enumerate}[label=\alph*.]
        \item The hyperparameter configuration I used for the fully connected output with 1 fully connected hidden layer was a learning rate of 0.01291549665014884 and $M = 600$.
    \end{enumerate}
\end{document}