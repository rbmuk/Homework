...
def compute_distances(X1, X2):
    ...
    dists = -2 * X1 @ X2.T + np.sum(X1**2, axis=1, keepdims=True) + np.sum(X2**2, axis=1).reshape(1, -1)
    dists = np.sqrt(dists)
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_trains = np.stack([np.vstack(X_train_folds[:i] + X_train_folds[i+1:]) for i in range(num_folds)])
    y_trains = np.stack([np.hstack(y_train_folds[:i] + y_train_folds[i+1:]) for i in range(num_folds)])
    X_vals = np.stack([X_train_folds[i] for i in range(num_folds)])
    y_vals = np.stack([y_train_folds[i] for i in range(num_folds)])
