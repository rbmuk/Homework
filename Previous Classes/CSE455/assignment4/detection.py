...
def hog_feature(image, pixel_per_cell=8):
    ...
    hogFeature, hogImage = feature.hog(image, pixels_per_cell=(pixel_per_cell, pixel_per_cell), block_norm='L1', visualize=True, feature_vector=True)
...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    for i in range(0, H, stepSize):
        for j in range(0, W, stepSize):
            ...
            score = np.dot(base_score, hogFeature)
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for i in range(len(heatmaps)):
        heatmap += gaussian(heatmaps[i], sigma=sigmas[i])
