...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        self.mean = X.mean(axis=0) # feature mean
        X_centered = X - self.mean
        V, _ = self._svd(X_centered)
        self.W_pca = V
    ...
    def _eigen_decomp(self, X):
        ...
        e_vals, e_vecs = np.linalg.eig(1/(N-1) * X.T @ X)
    ...
    def transform(self, X, n_components):
        ...
        X_centered = X - self.mean
        X_proj = X_centered @ self.W_pca[:, :n_components]
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            scatter_within += X_i_centered.T @ X_i_centered
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            scatter_between += len(X_i) * np.outer(mu_i - mu, mu_i - mu)
