...
def rotate_image(input_image, theta):
    ...
    center = np.array([input_rows, input_cols]) / 2
    for i in range(input_rows):
        for j in range(input_cols):
            point = np.array([i, j])
            rotated_point = rotate2d(point - center, theta) + center
            rotated_i, rotated_j = rotated_point
            if 0 <= rotated_i < input_rows and 0 <= rotated_j < input_cols:
                output_image[i, j] = input_image[int(rotated_i), int(rotated_j)]
