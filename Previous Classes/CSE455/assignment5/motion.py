...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        A = np.hstack((Iy_window[:, np.newaxis], Ix_window[:, np.newaxis]))
        b = -It_window

        vy, vx = np.linalg.inv(A.T @ A) @ A.T @ b
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        A = np.hstack((Ix_window[:, np.newaxis], Iy_window[:, np.newaxis]))
        G = A.T @ A
        ...
        for k in range(num_iters):
            ...
            deltaI_k = img1[y1-w:y1+w+1, x1-w:x1+w+1] - img2[y2-w:y2+w+1, x2-w:x2+w+1]
            deltaI_k = deltaI_k.flatten()
            bk = [np.sum(deltaI_k * Ix_window), np.sum(deltaI_k * Iy_window)]
...
def pyramid_lucas_kanade(
    ...
    for L in range(level, -1, -1):
        ...
        if L > 0: # without this it does g^{-1} + d^0, when we want d_ret = g^0 + d^0
            g = scale * (g+d)
...
def IoU(bbox1, bbox2):
    ...
    intersection_coords = (max(x1, x2), max(y1, y2), min(x1+w1, x2+w2), min(y1+h1, y2+h2))
    intersection_area = (intersection_coords[2] - intersection_coords[0]) * (intersection_coords[3] - intersection_coords[1])
