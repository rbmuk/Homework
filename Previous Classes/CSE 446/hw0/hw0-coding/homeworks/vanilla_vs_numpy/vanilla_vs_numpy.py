...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    A_plus_a_trans = vanilla_transpose(A)
    for i in range(len(A)):
        for j in range(len(A)):
            A_plus_a_trans[i][j] += A[i][j]
    B_trans = vanilla_transpose(B)
    A_plus_A_trans_times_x = vanilla_matmul(A_plus_a_trans, x)
    B_trans_times_y = vanilla_matmul(B_trans, y)

    return [value + B_trans_times_y[i] for i, value in enumerate(A_plus_A_trans_times_x)]
