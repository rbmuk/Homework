...
\begin{document}
    ...
    We now prove the correctness of this algorithm. We claim that $EA[n]$ and $EB[n]$ represent the number of valid strings with $n$ letters that end in $A$ and $B$ respectively. Clearly the claim is true for $n=1$ beacuse $k_A, k_B \geq 1$ and there is precisely 1 string that ends in $A$, $B$ respectively. Suppose the claim is true for $<n$. If $S$ is a valid string with $n$ letters ending in $A$, then the consecutive subsequence containing the last $A$ has length $1 \leq \ell \leq k_A$. The final observation is that the number of valid strings on $n$ letters which has a length $\ell$ consecutive subsequence as 0s on the right is the same as the number of strings on $n-\ell$ letters ending in $B$. Appying the inductive hypothesis now shows that $EA$ has the correct value. Applying the exact same logic shows that $EB$ has the correct value, so our inductive step. Finally, every valid subsequence on $n$ letters either ends in $A$ or $B$, which proves the correctness of the algorithm.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic} % The number indicates line number visibility
            \Procedure{Optimal-Village-Cost}{$x_1, \ldots, x_n, K$}
                ...
                \State $A \gets \text{an } n \times n \times K \text{ tensor filled with } \infty$
                \State $Closer \gets \text{an } n \times n \times n \text{ matrix filled with } 0$
                \State $D \gets \text{an } n \times n \text{ matrix filled with } 0$
                \For{$1 \leq i \leq n$}
                    \For{$1 \leq l < u \leq n$}
                ...
                \EndFor
                \For{$1 \leq i < j \leq n$}
                    \State $Closer[i,j] \gets \displaystyle i - \min\SET{\ell : d(x_\ell,x_i) \leq d(x_\ell, x_j)}$
                \EndFor
                \For{$1 \leq i < j \leq n$}
                    \State $A[i,j,1] \gets \sum_{\ell=1}^i |x_\ell - x_j|$
                \EndFor
                \For{$k = 2 \text{ to } K$}
                    \For{$k \leq i, j \leq n$}
                        \State $A[i,j,k] \gets \displaystyle \min_{1 \leq \ell < j} \SET{A[i-Closer[j,\ell], \ell,k-1] + D[j, j-Closer[j,\ell], i]}$
                    \EndFor
                \EndFor
                \State \Return $\min_{K \leq j \leq n} A[n,j,K]$
            \EndProcedure
        \end{algorithmic}
    \end{algorithm}

    First observe that for $i > j$, $Closer[i,j]$ is the number of villages between $i$ and $j$ that are closer to $i$ than $j$. I claim that $A[i,j,k]$ equals the sum of the distances to each village to its nearest post office using only the first $i$ villages with the rightmost post office in village $j$ where we are allocated $k$ post offices. We prove this by induction on $k$. The claim is true for $k = 1$ because when we only have one post office, the index $j$ specifies where it is and then we just set $A[i,j,1]$ equal to the sum of the distances from the $i$ villages to the post office. Now suppose the claim is true for $<k$. 

    I claim that the optimal post office placement on $i$ villages, when we are allocated $k$ post offices, and when the rightmost post office is at position $j$ can be found using formula in the algorithm. This is because we can partition over where the second rightmost post office is. When the second rightmost post office is at position $\ell$, the villages that overlap between $\ell$ and $j$ will precisely be the ones between them. In this case, those that are closer to $j$ will be served by the post office at $j$, and those that are closer to $\ell$ will be served by the post office at $\ell$. So to find the optimal post office placement with rightmost position $j$, we just need to optimize over $\ell$, the second rightmost post office location, on only the villages that are benefitting from $\ell$--i.e. those that are closer to $\ell$ than to $j$. After this we add the distances from the villages closest to $j$ (Note: if $j$ is the rightmost post office than anything to the right of it is certainly closest to $j$ because the $x_i$ are sorted), which gives us the sum of the distances from any post office to the villages it serves. This completes the proof of the claim. 
