...
\begin{document}
    ...
    \begin{enumerate}[leftmargin=\labelsep]
        ...
        \end{align*}
        Our solution is now clear: We can just run Dijkstra's algorithm, but replace the weight of each edge by the log of its weight. We have the following algoirthm:
        ...
        \begin{algorithm}[H]
            ...
            \Fn{\Dijkstra{$G$, $s$}}{
                ...
                Initialize an empty dictionary $dist$\;
                Initialize an empty dictionary $prev$\;
                \For{each vertex $v$ in $G$}{
                ...
                $Q$.enqueue($s$, 0)\;
                \While{$Q$ is not empty}{
                    $u, d \leftarrow Q$.dequeue()\;
                    \If{$d > dist[u]$}{
                        \textbf{continue}\;
                    }
                    \For{each neighbor $v$ of $u$}{
                        $alt \leftarrow dist[u] + \log(w_{(u, v)})$\;
                        \If{$alt < dist[v]$}{
        ...
        \end{algorithm}
        This is just Dijkstras algorithm, with an $O(1)$ change in calculating the weight of an edge, so the running time is still $O(n+m)$. Our algorithm is correct by the correctness of Djikstra's algorithm and the observations we made above.
    \end{enumerate}
\end{document}