...
\begin{document}
    ...
    \begin{enumerate}[label=\alph*)]
        \item This algorithm has running time $T(n) = 7T(n/2) + O(n)$. The corresponding tree is:
        \[\begin{tikzcd}
            &&& n &&&& n \\
            {n/2} & {n/2} & {n/2} & {n/2} & {n/2} & {n/2} & {n/2} & {\frac{7^1}{2^1}n} \\
            \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & {\frac{7^i}{2^i}n}
            ...
            \arrow[from=1-4, to=2-7]
        \end{tikzcd}\]
        As the height of this tree is clearly $\log n$, we get $T(n) = \sum_{i=0}^{\log_2 n} \qty(\frac{7}{2})^i n$. We see that
        \begin{align*}
            \sum_{i=0}^{\log_2 n} \qty(\frac{7}{2})^i n &= n\sum_{i=0}^{\log_2 n} \qty(\frac{7}{2})^i \approx n(3.5)^{\log_2 n} \leq n \cdot n^{1/\log_{3.5}(2)} = O(n^{2.81})
        \end{align*}
        Where we used that $\log_2(n) = \frac{\log_7(n)}{\log_7(2)} \leq 2.28\log_7(n)$. Since $1/\log_7(2) \leq 2.28$.

        \item This translates in symbols to $T(n) = 25T(n/5) + n^2$. It is clear to see that the weight of the $i$th level of the tree for $i \geq 1$ would be $\frac{25^i}{5^i} n^2 = 5^in^2$. Thus we get
        \begin{align*}
            T(n) = \sum_{i=0}^{\log_5 n} 5^in^2 = n^2\sum_{i=0}^{\log_5 n} 5^i \approx n^2\cdot 5^{\log_5 n} = n^3
        \end{align*}

        \item This algorithm translates to 
        \begin{align*}
            T(n) = 4T(n-4) + O(1)
        \end{align*}
        ...
        \begin{figure}[H]
            ...
            \begin{tikzcd}
                && n &&& 1 \\
                {n-4} & {n-4} && {n-4} & {n-4} & {4^1 \cdot 1} \\
                \vdots & \vdots && \vdots & \vdots & {4^i \cdot 1}
            ...
            \end{tikzcd}
        \end{figure}
        The height of this tree is the number of subtractions of 4 from $n$ to get $0$. This is clearly $\frac{n}4$. Thus $T(n) = \sum_{i=0}^{n/4} 4^i = O(4^{n/4})$.
