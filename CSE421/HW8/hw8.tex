...
\begin{document}
    ...
    If we were to write an integer program for this problem, we would do so by doing the following:

    For each $1 \leq i \leq m$, let $x_i \in \SET{0, 1}$ represent if we include $S_i$ or not. Then the cost of the family is $\sum_{i=1}^m x_ic_i$. Since we want each element in $\SET{1, \ldots, n}$ to be in at least one of the sets in our family, we need that 
    \begin{align*}
        \sum_{i : j \in S_i} x_i \geq 1
    \end{align*}
    Meaning that we need to include at least one of the sets including $j$ for each $j$. Thus the integer program is the following:
    \begin{align*}
        \min \quad &\sum_{i=1}^m x_ic_i \\
        \text{s.t.,} \quad &\sum_{i : j \in S_i} x_i \geq 1 \quad \forall 1 \leq j \leq n \\
        &x_i \in \SET{0, 1} \quad \forall 1 \leq i \leq m
    \end{align*}

    Thus part (a) follows since the LP in the question is just the relaxation of the integer condition $x_i \in \SET{0, 1}$ to $x_i \geq 0$.
