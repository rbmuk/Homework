...
def harris_corners(img, window_size=3, k=0.04):
    ...
    d2y = dy ** 2
    dxdy = dx * dy
    M11 = convolve(d2x, window, mode='constant', cval=0)
    M22 = convolve(d2y, window, mode='constant', cval=0)
    M12 = convolve(dxdy, window, mode='constant', cval=0)
    detM = M11 * M22 - M12 ** 2
    traceM = M11 + M22
...
def simple_descriptor(patch):
    ...
    feature = (patch - np.mean(patch)) / np.std(patch)
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    index_of_min_dists = np.argmin(dists, axis=-1)
    dists_sorted = np.sort(dists)
    matches = np.argwhere(dists_sorted[:, 0] < threshold * dists_sorted[:, 1])
    matches = np.concatenate((matches, index_of_min_dists[matches]), axis=1)
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    # Note: while there are many ways to do random sampling, we use
    ...
    for i in range(n_iters):
        ...
        # 2. Find H for these samples
        H = np.linalg.lstsq(sample2, sample1, rcond=None)[0]
        H[:, 2] = np.array([0, 0, 1])

        # 3. Calculate distance between the points
        new_points = matched2 @ H
        dists = np.linalg.norm(matched1 - new_points, axis=1)
        inliers = dists <= threshold
        if sum(inliers) > n_inliers:
            max_inliers = inliers
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
        for j in range(cols):
            G_cell = G_cells[i,j].ravel()
            theta_cell = theta_cells[i,j].ravel()
            for G, theta in zip(G_cell, theta_cell):
                bin_index = int(theta // degrees_per_bin) % n_bins
                hists[i, j, bin_index] += G
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    Hs = [np.eye(3)]  # Hs[i] changes imgs[i] to imgs[i-1]
    for i in range(len(imgs) - 1):
        Hs.append(ransac(keypoints[i], keypoints[i+1], matches[i])[0])

    ref_H = [np.eye(3)]
    for i in range(1, len(imgs)):
        ref_H.append(ref_H[i-1] @ Hs[i])

    output_shape, offset = get_output_space(imgs[0], imgs, ref_H)
    ...
    for i in range(0, len(imgs)):
        imgs_warped.append(warp_image(imgs[i], ref_H[i], output_shape, offset))
        masks.append(imgs_warped[i] != -1)
        imgs_warped[i][~masks[i]] = 0

    merged = np.sum([imgs_warped[i] for i in range(len(imgs_warped))], axis=0)
    overlap = np.sum([masks[i] * 1.0 for i in range(len(masks))], axis=0)

    ret = merged / np.maximum(overlap, 1)
